...
{
    source_line_t *tmp = calloc(1, sizeof(*tmp));
    if (tmp == NULL)
        abort();
...
/* File c/getinput.c can used to handle standard input */
int main(int argc, char **argv)
{
    ...
    int rc;
    file_t file;
    if ((rc = file_read(&file, argv[1])) == -1)
        return 1;
    else if (rc == 0)
        printf("warning: no newline at end of file\n");
    ...
    for (size_t i = 0; i < file.source.len; i++) {
        const char *buf = (const char *)file.source.lines[i].buffer;
        printf("  %*zu |  ", 3, i + 1);
        printf("%s", buf);
        fflush(stdout);
    }
